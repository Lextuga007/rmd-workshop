...
Note this will over-write the existing Rmd file of the same name
...
```{r eval=FALSE}
for (f in files) 
  rmarkdown::render(paste0("03-images/", f))
```
...
source("R/get-bakeoff-ratings.R")
...
SQL chunks do not support the same options as R but these can be set globally.
...
`r emo::ji("exclamation")` **Limitations of SQL** CTEs are not always supported, depending on what package you use
