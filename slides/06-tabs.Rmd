...
1. Add a header `# Series Analysis`.

1. Change `# Overview` to `### Overview {.tabset}`.
...
1. We'll add a new header `## Days {.tabset .tabset-pill}`
...
```markdown 
viewers <- viewers %>% select(days) %>% unique() %>% pull()
```
