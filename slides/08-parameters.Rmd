...
1. Change the `input` parameter from slider to numeric.

1. Install a colour package like [wesanderson](https://github.com/karthik/wesanderson) or [popthemes](https://github.com/johnmackintosh/popthemes)

1. Add a new parameter called `pallete` 

1. This will require `choices: [choice1, choice2, choice3]` in the params

1. Add code to the ggplot2 to change the output colours: +
  `scale_colour_manual(values = wes_palette(params$palette))` or
  `scale_colour_manual(values = pop_palette(params$palette))`

use the parameter in our ggplot2 code.
...
```yaml
  palette: 
    ...
    value: aqua
    input: select
    choices: [aqua, bangles, beck, boo, bwitched]
```
